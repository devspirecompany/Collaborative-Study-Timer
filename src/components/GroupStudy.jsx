...
const GroupStudy = () => {
  ...
  // Mock available rooms (replace with actual API call later)
  useEffect(() => {
    // Simulate available rooms
    setAvailableRooms([
      { id: 1, name: 'Math Study Group', subject: 'Mathematics', players: 3, maxPlayers: 10, host: 'John' },
      { id: 2, name: 'Physics Review', subject: 'Physics', players: 5, maxPlayers: 10, host: 'Sarah' },
      { id: 3, name: 'Chemistry Lab', subject: 'Chemistry', players: 2, maxPlayers: 4, host: 'Mike' },
      { id: 4, name: 'Biology Study', subject: 'Biology', players: 1, maxPlayers: 4, host: 'Emma' },
      { id: 5, name: 'History Discussion', subject: 'History', players: 1, maxPlayers: 15, host: 'David' },
      { id: 6, name: 'English Literature', subject: 'English', players: 1, maxPlayers: 5, host: 'Lisa' },
    ]);
  }, []);
                  ...
                  {availableRooms.length === 0 ? (
                    ...
                    availableRooms.map((room) => (
                      <div 
                        key={room.id}
                        onClick={() => {
                          // In real app, join this specific room
                          setShowRoomBrowserModal(false);
                          showNotificationModal(`Joining ${room.name}...`, 'success');
                        }}
                        style={{
                          display: 'flex',
                          alignItems: 'center',
