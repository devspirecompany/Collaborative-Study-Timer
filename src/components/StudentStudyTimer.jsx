...
const StudentStudyTimer = () => {
  ...
  // Hide notification when file is selected
  useEffect(() => {
    if (showNotification && selectedReviewer) {
      setShowNotification(false);
    }
  }, [selectedReviewer, showNotification]);

  // Hide notification when switching to break mode
  useEffect(() => {
    if (showNotification && currentMode !== 'study') {
      setShowNotification(false);
    }
  }, [currentMode, showNotification]);
  ...
  const cleanContent = (content) => {
    ...
    // Common Tagalog words and phrases to remove
    const tagalogWords = [
      /\b(ang|ng|sa|ay|na|mga|ito|iyan|iyon|siya|kami|tayo|kayo|sila|ako|ikaw|mo|niya|namin|ninyo|nila|ko|nito|niyan|noon)\b/gi,
      /\b(at|o|kung|kapag|dahil|kaya|pero|ngunit|gayunman|subalit)\b/gi,
      /\b(may|mayroon|wala|hindi|oo|hindi|opo|hindi po)\b/gi,
      /\b(paano|ano|sino|saan|kailan|bakit|alin|gaano|ilan)\b/gi,
      /\b(tulad|parang|gaya|katulad|kapareho)\b/gi,
      /\b(maganda|masama|mabuti|masama|malaki|maliit|mataas|mababa)\b/gi,
      /\b(kumain|inom|tulog|gising|trabaho|aral|basa|sulat)\b/gi,
      /\b(mahal|mahal kita|salamat|salamat po|walang anuman|paalam)\b/gi,
      /\b(kumusta|kamusta|okay|okey|sige|tama|mali)\b/gi,
      /\b(ngayon|bukas|kahapon|noon|dati|minsan|palagi|hindi)\b/gi,
      /\b(dito|doon|diyan|rito|roon|riyan)\b/gi,
      /\b(kaya|kasi|dahil|kung|kapag|habang|bago|pagkatapos)\b/gi,
      /\b(para|para sa|tungkol|tungkol sa|hindi|hindi ba)\b/gi,
      /\b(may|mayroon|wala|hindi|oo|hindi|opo|hindi po)\b/gi,
      /\b(paano|ano|sino|saan|kailan|bakit|alin|gaano|ilan)\b/gi,
    ];
    ...
    // Remove Tagalog words
    tagalogWords.forEach(pattern => {
  ...
  const startTimer = () => {
    // If study mode and no reviewer selected, scroll to file selection button
    if (currentMode === 'study' && !selectedReviewer) {
      const selectFileButton = document.getElementById('select-file-button');
      
      if (selectFileButton) {
        // Add pop-up animation to the button
        selectFileButton.classList.add('pop-up-animation');
        
        // Scroll to the button
        selectFileButton.scrollIntoView({ 
          behavior: 'smooth', 
          block: 'center' 
        });
        ...
        setTimeout(() => {
          selectFileButton.classList.remove('pop-up-animation');
        }, 1000);
        
        // Show notification (no animation on container, no auto-close)
        setNotificationMessage('Please select a file before starting the timer');
  ...
  return (
    <div>
     <Sidebar/>
      ...
      {/* Main Content */}
      <main className="main-content" style={{ paddingTop: showNotification ? '70px' : '0' }}>
            ...
            <div className="timer-mode-selector">
              ...
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M12 2C8 2 5 5 5 9C5 11.5 6.5 13.5 8.5 14.5C9 14.7 9.2 15.2 9.1 15.7L8.8 17.2C8.7 17.7 9.1 18 9.6 18H14.4C14.9 18 15.3 17.7 15.2 17.2L14.9 15.7C14.8 15.2 15 14.7 15.5 14.5C17.5 13.5 19 11.5 19 9C19 5 16 2 12 2Z"/>
                  <path d="M9 21H15"/>
                  <path d="M10 18V21"/>
                  <path d="M14 18V21"/>
                  ...
                  <div style={{ marginBottom: '1rem' }}>
                    ...
                    >
                      {selectedReviewer.reviewContent 
                        ? cleanContent(selectedReviewer.reviewContent.substring(0, 2000)) + (selectedReviewer.reviewContent.length > 2000 ? '...' : '')
                        : selectedReviewer.content 
                          ? cleanContent(selectedReviewer.content.substring(0, 2000)) + (selectedReviewer.content.length > 2000 ? '...' : '')
                ...
                {selectedReviewer.keyPoints && selectedReviewer.keyPoints.length > 0 && (
                  <div style={{ 
                    ...
                    <ul style={{ 
                      ...
                      {selectedReviewer.keyPoints.slice(0, 5).map((point, idx) => (
                        <li key={idx} style={{ 
                          marginBottom: idx < Math.min(selectedReviewer.keyPoints.length, 5) - 1 ? '0.75rem' : '0', 
                          paddingLeft: '1.75rem',
                          position: 'relative',
                          color: 'var(--text-primary)',
                          fontSize: '0.875rem',
                ...
                <>
                  {Object.keys(reviewers).length === 0 ? (
                    <div className="empty-files-state" style={{ 
                      ...
                      <h3 style={{ margin: 0 }}>No reviewers available</h3>
                      <p style={{ margin: 0 }}>Upload files or create reviewers in "My Files" to start studying</p>
                      <button 
      ...
      {showNotification && (
        <div 
          ...
          style={{
            ...
            borderBottom: `2px solid ${notificationType === 'success' ? 'rgba(16, 185, 129, 0.5)' : 'rgba(239, 68, 68, 0.5)'}`,
            padding: '1rem 2rem',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
