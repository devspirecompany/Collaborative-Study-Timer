...
const MyFiles = () => {
  ...
  const [uploadSuccessMessage, setUploadSuccessMessage] = useState({ fileName: '', fileType: '', folder: '' });
  ...
  const handleQuickCreateFolder = async () => {
    ...
    try {
      const folderNameLower = quickFolderName.trim().toLowerCase();
      ...
      if (response.success) {
        const createdFolderName = quickFolderName.trim();
        setFolders(prev => [...prev, createdFolderName]);
        setFilesBySubject(prev => ({
          ...prev,
          [createdFolderName]: []
        }));
        // Auto-select the newly created folder in upload
        setUploadFolder(createdFolderName);
                    ...
                    {showQuickCreateFolder ? (
                      <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                        <input
                          ...
                          onKeyPress={(e) => {
                            if (e.key === 'Enter' && quickFolderName.trim()) {
                        ...
                        <button
                          ...
                          disabled={!quickFolderName.trim() || isCreatingFolder}
                          style={{
                            ...
                            fontSize: '0.9rem',
                            fontWeight: '600',
                            cursor: quickFolderName.trim() && !isCreatingFolder ? 'pointer' : 'not-allowed',
                            opacity: quickFolderName.trim() && !isCreatingFolder ? 1 : 0.5
                          }}
                        >
      ...
      {showUploadSuccessModal && (
        <div className="modal-overlay">
          <div className="success-modal">
            ...
            <h2 style={{ textAlign: 'center' }}>✅ File Uploaded Successfully!</h2>
            <div style={{ 
              marginTop: '1rem', 
              marginBottom: '1.5rem',
              padding: '1rem',
              ...
              <p style={{ margin: '0.5rem 0', fontSize: '0.95rem', color: 'var(--text-secondary)', textAlign: 'center' }}>
                <strong style={{ color: 'var(--text-primary)' }}>{uploadSuccessMessage.fileType}</strong> file
              </p>
              <p style={{ margin: '0.5rem 0', fontSize: '1rem', color: 'var(--text-primary)', fontWeight: '600', textAlign: 'center' }}>
                "{uploadSuccessMessage.fileName}"
              </p>
              <p style={{ margin: '0.5rem 0', fontSize: '0.9rem', color: 'var(--text-secondary)', textAlign: 'center' }}>
                Uploaded to <strong style={{ color: 'var(--primary-light)' }}>{uploadSuccessMessage.folder}</strong>
              </p>
            </div>
            <button
